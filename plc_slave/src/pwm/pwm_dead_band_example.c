...
int FPwmDeadBandExample()
{
    ...
    db_cfg.db_rise_cycle = 0;
    db_cfg.db_fall_cycle = 0;
    ...
    /*enable pwm*/
    FPwmEnable(&pwm_ctrl, PWM_CHANNEL_USE);
    FPWM_DEBUG("pwm_pulse = %d, pwm_period=%d\n", pwm_cfg.pwm_pulse, pwm_cfg.pwm_period);
    // fsleep_seconds(1000);

    // pwm_cfg.tim_ctrl_div = 1000-1;
    // pwm_cfg.pwm_period = 480;
    // pwm_cfg.pwm_pulse = 120;

    
    // FPwmVariableSet(&pwm_ctrl, PWM_CHANNEL_USE, &pwm_cfg);
    // FPwmEnable(&pwm_ctrl, PWM_CHANNEL_USE);
    // printf("pwm_pulse = %d, pwm_period=%d\n", pwm_cfg.pwm_pulse, pwm_cfg.pwm_period);
    // fsleep_seconds(500);
...
exit:
    ...
    {
        printf("%s@%d: pwm dead band example [failure], ret = %d \r\n", __func__, __LINE__, ret);
    }
