...
/************************** Constant Definitions *****************************/
static u8 send_buffer[TEST_BUFFER_SIZE] = {0}; /* poll buffer for transmitting data */
static u8 recv_buffer[TEST_BUFFER_SIZE] = {0}; /* poll buffer for receiving data */
const char* ceshi = "AT\r\n";
const char* reset = "AT+RESET\r\n";
const char* yali = "AT+F\r\n";  //0.00～20.00
...
{    
    ...
    //     recv_buffer[i] = 0;
    // }
    cpy_length = strlen(ceshi);
    printf("cpy_num %d",cpy_length);
    strncpy((char*)send_buffer,ceshi,cpy_length);
    // send_buffer[TEST_BUFFER_SIZE - 1] = '\0';
    
    // printf("UART send A~%c characters. \r\n", 'A' + TEST_BUFFER_SIZE - 1);
    // printf("UART recv begin, you can put %d characters from uart to test. \r\n", TEST_BUFFER_SIZE);
    total_send_count = FPl011Send(&uart_instance, send_buffer,cpy_length);
    printf("total_send %d \n" , total_send_count);
    // while ((total_error_count == 0) && ((total_send_count != TEST_BUFFER_SIZE) ))
    // {
    //     ;
    // }

    total_received_count = FPl011Receive(&uart_instance, recv_buffer, TEST_BUFFER_SIZE);
...
int FSerialMioExample()
{
    ...
    {
        printf("%s@%d: Serial mio example [failure].\r\n", __func__, __LINE__);
    }
